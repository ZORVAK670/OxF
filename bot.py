...
def main_menu_keyboard(lang: str) -> ReplyKeyboardMarkup:
    rows = [
        [KeyboardButton(t("menu_tasks", lang)), KeyboardButton(t("menu_join_channel", lang))],
        [KeyboardButton(t("menu_daily_bonus", lang)), KeyboardButton(t("menu_referral", lang))],
        [KeyboardButton(t("menu_balance", lang)), KeyboardButton(t("menu_withdraw", lang))],
        [KeyboardButton(t("menu_watch_ads", lang))],
        [KeyboardButton(t("menu_support", lang))],
...
async def balance_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    u = db.get_user(user_id)
    lang = u["lang"]
    usdt = round(u["points"] / config.POINTS_PER_USDT, 4)
    await update.message.reply_text(t("balance_msg", lang, points=u["points"], usdt=usdt))


async def daily_bonus_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await ensure_joined(update, context):
        return
    user_id = update.effective_user.id
    u = db.get_user(user_id)
    ...
    now = int(time.time())
    if now - u["last_daily_bonus"] < 24 * 3600:
        await update.message.reply_text(t("daily_bonus_wait", lang))
        return
    db.add_points(user_id, config.POINTS_DAILY_BONUS)
    db.set_last_daily_bonus(user_id, now)
    await update.message.reply_text(t("daily_bonus_ok", lang, points=config.POINTS_DAILY_BONUS))


async def referral_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await ensure_joined(update, context):
        return
    user_id = update.effective_user.id
    u = db.get_user(user_id)
...
async def tasks_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await ensure_joined(update, context):
        return
    user_id = update.effective_user.id
    lang = user_lang(user_id)
    ...
    if not tasks:
        ...
        return
    for task in tasks:
        if db.has_completed_task(user_id, task["task_id"]):
            continue
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton(t("verify_btn", lang), callback_data=f"verify_{task['task_id']}")
        ]])
...
async def watch_ads_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not data:
        await update.message.reply_text(t("no_ads_available", lang))
        return
    ...
    try:
        if image_url:
            await update.message.reply_photo(image_url, caption=caption, reply_markup=kb, parse_mode=ParseMode.HTML)
        else:
...
async def support_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = user_lang(user_id)
    contact = f"@{config.SUPPORT_USERNAME}" if config.SUPPORT_USERNAME else "-"
    await update.message.reply_text(t("support_msg", lang, contact=contact))


async def change_lang_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(t("choose_language", "en"), reply_markup=lang_keyboard())


async def withdraw_start_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await ensure_joined(update, context):
        return ConversationHandler.END
    user_id = update.effective_user.id
    lang = user_lang(user_id)
...
async def menu_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Routes presses on the persistent reply-keyboard menu buttons."""
    ...
    def matches(key: str) -> bool:
        return text in [t(key, l) for l in langs]

    if matches("menu_tasks") or matches("menu_join_channel"):
        await tasks_message(update, context)
    elif matches("menu_daily_bonus"):
        await daily_bonus_message(update, context)
    elif matches("menu_referral"):
        await referral_message(update, context)
    elif matches("menu_balance"):
        await balance_message(update, context)
    elif matches("menu_watch_ads"):
        await watch_ads_message(update, context)
    elif matches("menu_support"):
        await support_message(update, context)
    elif matches("menu_language"):
        await change_lang_message(update, context)
...
def main():
    ...
    withdraw_texts = [t("menu_withdraw", l) for l in ("fa", "ps", "en")]
    withdraw_pattern = "^(" + "|".join(re.escape(x) for x in withdraw_texts) + ")$"

    withdraw_conv = ConversationHandler(
        entry_points=[
            CallbackQueryHandler(withdraw_start, pattern="^withdraw_start$"),
            MessageHandler(filters.Regex(withdraw_pattern), withdraw_start_message),
        ],
        states={
